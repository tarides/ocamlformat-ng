...
let mapper =
  ...
  let attributes (m : Ast_mapper.mapper) (attrs : attribute list) =
    let attrs =
      if not !ignore_docstrings then attrs
      else
        List.filter
          (function
  ...
  let expr (m : Ast_mapper.mapper) exp =
    ...
    | Pexp_sequence
        ...
        m.expr m (Exp.sequence e1 (Exp.sequence ~attrs:pexp_attributes e2 e3))
...
let check_same_ast ~impl s1 s2 =
  ...
  if impl then (
    let ast1 = Parse.implementation lex1 |> mapper.structure mapper in
    Location.input_name := "/tmp/out.txt";
    let ast2 = Parse.implementation lex2 |> mapper.structure mapper in
    ast1 = ast2)
  else
    let ast1 = Parse.interface lex1 |> mapper.signature mapper in
    Location.input_name := "/tmp/out.txt";
    let ast2 = Parse.interface lex2 |> mapper.signature mapper in
    ast1 = ast2
