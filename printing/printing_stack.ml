...
type core_type_elt = Arrow | Tuple
...
let needs_parens elt parents =
  ...
  | Core_type (Ptyp_arrow _) | Core_type (Ptyp_tuple _) ->
      List.exists
        (function
          | Core_type
              (Ptyp_constr _ | Ptyp_class _ | Ptyp_arrow _ | Ptyp_tuple _) ->
              true
          | _ -> false)
        parents
  ...
  | Pattern (Ppat_variant (_, Some _)) | Pattern (Ppat_construct (_, Some _)) ->
      List.exists
        (function
          ...
          | Function_parameter | Value_binding ->
              true
  ...
  | Cons_constr _ ->
      List.exists
        (function
          | Pattern
              (Ppat_lazy _ | Ppat_construct _ | Ppat_variant _ | Ppat_open _)
          | Function_parameter | Value_binding ->
              true
          ...
          | Expression
              ( Pexp_field _ | Pexp_setfield _ | Pexp_access _
              | Pexp_dotop_access _ | Pexp_apply _ | Pexp_construct _
              | Pexp_variant _ | Pexp_assert _ | Pexp_lazy _ ) ->
              true
  ...
  | Pattern (Ppat_tuple _) ->
      List.exists
        (function
          | Pattern
              ( Ppat_tuple _ | Ppat_construct _ | Ppat_variant _ | Ppat_lazy _
              | Ppat_exception _ | Ppat_open _ )
          | Cons_constr _ | Function_parameter | Value_binding ->
              true
          | _ -> false)
        parents
  | Pipe _ | Pattern (Ppat_or _) ->
      List.exists
        (function
          ...
          | Pattern _ | Attribute | Function_parameter | Value_binding
          | Cons_constr _ ->
              true
  ...
  | Pattern (Ppat_alias _) ->
      List.exists
        (function
          | Pattern _ | Function_parameter | Value_binding | Cons_constr _ ->
              true
          | _ -> false)
        parents
  ...
  | Expression (Pexp_lazy _) ->
      List.exists
        (function
          ...
          | Class_expr (Pcl_apply _)
          | Expression
              ( Pexp_apply _ | Pexp_construct _ | Pexp_variant _ | Pexp_assert _
              | Pexp_lazy _ ) ->
              true
          | Expression (Pexp_record _) | Unpack ->
              (* Not described by the precedence table, but won't parse otherwise. *)
  ...
  | Last_if_branch ->
      List.exists
        (function
          ...
          | Class_expr (Pcl_apply _)
          | Expression
              ( Pexp_apply _ | Pexp_assert _ | Pexp_lazy _ | Pexp_construct _
              | Pexp_variant _ | Pexp_record _ | Pexp_array _ | Pexp_tuple _
              | Pexp_sequence _ | Pexp_send _ )
          | Then_branch | Record_field | Unpack ->
              true
          | _ -> false)
        parents
  ...
  | Expression (Pexp_while _) ->
      List.exists
        (function
          | Prefix_op | Infix_op _
          | Class_expr (Pcl_apply _)
          | Expression
              ( Pexp_apply _ | Pexp_assert _ | Pexp_lazy _ | Pexp_construct _
              | Pexp_variant _ | Pexp_send _ )
          | Record_field ->
              true
          | _ -> false)
        parents
  | Expression (Pexp_sequence _) ->
      List.find_map
        (function
          | Attribute | Prefix_op | Infix_op _
          | Class_expr (Pcl_apply _)
          | Expression
              ( Pexp_apply _ | Pexp_assert _ | Pexp_lazy _ | Pexp_construct _
              | Pexp_variant _ | Pexp_record _ | Pexp_array _ | Pexp_ifthen _
              | Pexp_ifthenelse _ | Pexp_list_lit _ | Pexp_sequence _
              | Pexp_send _ | Pexp_tuple _ )
          | Last_if_branch | Then_branch | Record_field | Unpack ->
              Some true
  ...
  | Class_expr (Pcl_fun _) | Class_expr (Pcl_let _) | Class_expr (Pcl_open _) ->
      List.exists
        (function Attribute | Class_expr (Pcl_apply _) -> true | _ -> false)
        parents
  | Expression (Pexp_letopen _) | Expression (Pexp_let _) ->
      List.exists
        (function
          ...
          | Expression
              ( Pexp_apply _ | Pexp_construct _ | Pexp_variant _ | Pexp_assert _
              | Pexp_lazy _ | Pexp_record _ | Pexp_tuple _ | Pexp_field _
              | Pexp_access _ | Pexp_dotop_access _ | Pexp_setfield _
              | Pexp_sequence _ ) ->
              true
          | Prefix_op
          | Infix_op { on_left = true; _ }
  ...
  | Expression (Pexp_match _) | Expression (Pexp_try _) ->
      List.exists
        (function
          ...
          | Expression
              ( Pexp_apply _ | Pexp_construct _ | Pexp_variant _ | Pexp_assert _
              | Pexp_lazy _ | Pexp_match _ | Pexp_try _ | Pexp_function _
              | Pexp_list_lit _ | Pexp_sequence _ | Pexp_record _ | Pexp_tuple _
                )
          | Prefix_op
          | Infix_op { on_left = true; _ }
          | Cons_constr { on_left = true }
