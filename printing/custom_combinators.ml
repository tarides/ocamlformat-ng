...
module Enclosed_separated = struct
  type raw = t * t list
  type element = { doc : t; has_semi : bool }
  ...
  module Wrapped : sig
    ...
    let rec insert_semi : element list -> t list = function
      ...
      | [ x ] -> [ x.doc ]
      | x1 :: (x2 :: _ as xs) ->
          if x1.has_semi then x1.doc :: insert_semi xs
          else
    ...
    let rec collate ?(first = false) = function
      ...
      | x :: xs ->
          let x =
            if first then group (x ^^ break 1)
            else group (break 0 ^^ x ^^ break 1)
          in
          x ^^ collate xs
  ...
  module Fit_or_vertical : sig
    ...
    let rec pp_fields_aux = function
      ...
      | x :: (_ :: _ as rest) ->
          let elt = fmt x in
          if elt.has_semi then elt.doc ^/^ pp_fields_aux rest
          else
