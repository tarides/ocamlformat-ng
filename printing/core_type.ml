...
module rec Polymorphic_variant : sig
  ...
  module Row_field : sig
    ...
    let pp_desc = function
      ...
      | Rtag (tag, has_empty_constr, p :: ps) ->
          ...
          let params =
            if not has_empty_constr then params
            else
  ...
  let pp_row_prefix ~prefix_if_necessary ~loc ~opening_token x xs =
    ...
    let opening_token =
      if not needs_prefix then opening_token
      else
...
and Core_type : sig
  ...
  let rec starts_with_obj core_type =
    ...
    | Ptyp_alias (lhs, _) | Ptyp_tuple (lhs :: _) | Ptyp_arrow ((_, lhs) :: _, _)
      ->
        starts_with_obj lhs
    | Ptyp_object (_, _) -> true
    | Ptyp_any | Ptyp_var _ | Ptyp_parens _ | Ptyp_tuple _ | Ptyp_arrow _
    | Ptyp_constr (_, _)
    | Ptyp_class (_, _)
    | Ptyp_variant (_, _, _)
    | Ptyp_poly _ | Ptype_poly _ | Ptyp_package _ | Ptyp_extension _ ->
        false
  ...
  let rec ends_in_obj core_type =
    ...
    | Ptyp_object (_, _) -> true
    | Ptyp_any | Ptyp_var _ | Ptyp_parens _
    | Ptyp_constr (_, _)
    | Ptyp_class (_, _)
    | Ptyp_alias (_, _)
    | Ptyp_variant (_, _, _)
  ...
  and pp_constr ~loc name args =
    ...
    match args with [] -> name | x :: xs -> pp_params ~loc x xs ^/^ name
