...
module Payload = struct
  ...
  let pp_core_type : (core_type -> Document.t) ref = ref (fun _ -> assert false)
  let ct_ends_in_obj : (core_type -> bool) ref = ref (fun _ -> assert false)

  let pp_expression : (expression -> Document.t) ref =
    ref (fun _ -> assert false)

  let pp_pattern : (pattern -> Document.t) ref = ref (fun _ -> assert false)
  ...
  let ptyp tag ct =
    let break_after x = if !ct_ends_in_obj ct then x ^^ break 1 else x in
...
let ats kind =
  ...
  | Attached_to_item | Attached_to_exception -> LBRACKETAT
...
let attach ?(spaces = 1) kind doc attrs =
  ...
  | Some { attr_payload; attr_loc; _ } -> (
      ...
      in
      match kind with
      | Attached_to_structure_item | Attached_to_exception ->
...
let has_non_doc = List.exists is_non_doc
let attach_to_item ?spaces doc = attach ?spaces Attached_to_item doc
let attach_to_top_item doc = attach Attached_to_structure_item doc

let extract_text ~item_start_pos =
  let rec aux acc = function
    | ({ attr_name = { txt = "ocaml.text" | "ocaml.doc"; _ }; attr_loc; _ } as
      attr)
      :: attrs
      when Source_parsing.Comments.compare_pos attr_loc.loc_start item_start_pos
           <= 0 ->
        aux (attr :: acc) attrs
