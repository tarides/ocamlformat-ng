...
let module_name { txt; loc } =
  match txt with None -> underscore ~loc | Some name -> string ~loc name
...
module Constant : sig
  ...
  let pp ~loc = function
    | Pconst_float (nb, suffix_opt) | Pconst_integer (nb, suffix_opt) ->
        let nb =
          match suffix_opt with None -> nb | Some s -> nb ^ String.make 1 s
        in
        string ~loc nb
    | Pconst_char c -> char ~loc c
    | Pconst_string (_, None) ->
        let s =
          Source_parsing.Source.source_between loc.loc_start loc.loc_end
        in
...
and Pattern : sig
  ...
  let rec pp ?indent
      { ppat_desc; ppat_attributes; ppat_loc; ppat_ext_attributes; _ } =
    let desc =
  ...
  and pp_tuple lst = group (tuple_fields pp (List.hd lst) (List.tl lst))

  and pp_list_literal ~loc elts =
    List_like.pp ~loc ~formatting:Wrap (* TODO: add an option *)
      ~left:LBRACKET ~right:RBRACKET (List.map pp elts)
...
and Application : sig
  ...
  let perhaps_pun id = function
    | Labelled { name; extra_info = `Previous_token loc } when name.txt = id.txt
      ->
        let tilde = string ~loc "~" in
        tilde ^^ str name
    | Optional { name; extra_info = `Previous_token loc } when name.txt = id.txt
      ->
  ...
  *)
  let get_delims ~ext_attrs:(extension, attrs) ~begin_end ~loc lbl kw body =
    ...
    let closing =
      if begin_end then break 1 ^^ Token.pp ~after:body ~inside:loc END
      else break 0 ^^ Token.pp ~after:body ~inside:loc RPAREN
    in
    (lbl, opening, closing)
  ...
  let argument (lbl, exp) =
    ...
    | {
     pexp_desc =
       Pexp_parens
         { exp = { pexp_desc = Pexp_function (c :: cs); _ } as exp; begin_end };
     pexp_attributes = [];
     pexp_loc = loc;
     pexp_ext_attributes = ext_attrs;
     _;
        ...
        let compact =
          match !Options.Match.compact with Multi -> false | _ -> true
        in
        let function_, cases =
    ...
    | _ ->
        ...
        if lbl == empty then exp else group (lbl ^^ break 0 ^^ exp)
    ...
    | {
     pexp_desc =
       Pexp_parens
         {
           exp =
             { pexp_desc = Pexp_function (c :: cs); pexp_attributes = []; _ } as
             exp;
           begin_end;
         };
     pexp_attributes = [];
    ...
    let doc =
      ...
      | Smart ->
          ...
          (* It would be nice if I could have "current indent + len_labels" ...
             maybe. *)
          (* HACKISH *)
          if nb_labels > 4 && len_labels > 8 then fit_or_vertical ()
          else
...
and Expression : sig
  ...
  and pp_match ?parens ~loc ~ext_attrs:(extension, attrs) arg = function
    ...
    | c :: cs -> (
        ...
        | Some (before, after) ->
            group
              (group (before ^/^ match_) ^^ nest 2 (break 1 ^^ arg) ^/^ with_)
            ^^ cases ^/^ after)
  ...
  and pp_try ~loc ~ext_attrs:(extension, attrs) arg cs =
    ...
    | [ c ] (* TODO: guard this layout under an option *) ->
        ...
        pprefix try_ arg ^/^ pprefix with_ lhs
        ^^ nest 2 (ifflat (break 1) hardline ^^ rhs)
    | c :: cs ->
        let compact =
          match !Options.Match.compact with
          | Multi -> false
  ...
  and pp_list_literal ~loc elts =
    ...
    List_like.pp ~loc ~formatting:Wrap (* TODO: add an option *)
      ~left:LBRACKET ~right:RBRACKET elts
  ...
  and pp_dotop_access ~loc accessed path op paren indices val_ =
    let indices =
      ...
      | idx :: ids ->
          List_like.pp_fields ~formatting:Wrap (pp idx) (List.map pp ids)
    in
  ...
  and pp_sequence ~ext_attrs:(extension, attrs) e1 e2 =
    ...
    let e2 = pp e2 in
    let semi =
      Keyword.decorate (Token.pp ~after:e1 ~before:e2 SEMI) ~extension attrs
    in
    let doc = if compact then e1 ^^ semi ^/^ e2 else e1 ^^ semi ^//^ e2 in
    doc
  ...
  and pp_for ~(loc : Location.t) ~ext_attrs:(extension, attrs) it start stop dir
      body =
  ...
  and pp_letmodule ~loc ~ext_attrs:(extension, attrs) name (params, typ, mexp)
      expr =
  ...
  and pp_object ~loc ~ext_attrs cs = Class_structure.pp ~loc ~ext_attrs cs
...
and Fun_param : sig
  ...
  let labeled_simple_pattern ~loc:sloc lbl default pat_and_type parentheses =
    ...
    | Optional { name; extra_info = `Previous_token loc }, true, (None, ct), _
      ->
        ...
        let with_default =
          ...
          | Some exp ->
              let spaces =
                match exp.pexp_desc with Pexp_prefix_apply _ -> 1 | _ -> 0
              in
              let exp = Expression.pp exp in
    ...
    | ( Labelled { name; extra_info = `Single_token },
        ...
        None ) ->
        str name ^^ Pattern.pp pat
    | Nolabel, false, (Some pat, None), None -> Pattern.pp pat
    | _ -> assert false
  ...
  and attach_constraint mty is_first_cstr (kw, cstr) =
    let cstr =
      ...
      | Pwith_modtypesubst (lid, mty') ->
          ...
          Binding.pp_simple ~binder:COLONEQUAL ~keyword d1 d2
    in
    if is_first_cstr then prefix ~spaces:1 ~indent:2 mty cstr
    else
      let indent = match kw with With -> 2 | And -> 3 in
    ...
    let attributes =
      ...
      | attr :: attrs ->
          separate_map (break 0)
            ~f:(Attribute.pp Attached_to_structure_item)
            attr attrs
    in
...
and Module_declaration : sig
  ...
  let decide_context pmd_type : Binding.Module.context =
    ...
    match pmd_type.pmty_desc with Pmty_alias _ -> Struct | _ -> Sig
  ...
  let pp pmd =
    ...
    let text, pmd =
      let text, attrs =
        Attribute.extract_text ~item_start_pos:loc.loc_start pmd.pmd_attributes
      in
...
and Structure : sig
  ...
  let ends_in_obj lst =
    ...
    | Some { pstr_desc; _ } -> (
        ...
        | Pstr_eval _ | Pstr_value _ | Pstr_primitive _ | Pstr_module _
        | Pstr_recmodule _ | Pstr_modtype _ | Pstr_open _ | Pstr_include _
        | Pstr_attribute _ | Pstr_extension _ | Pstr_class _ | Pstr_class_type _
          ->
            false)
  ...
  let pp_include ~ext_attrs:(extension, attrs)
    ...
    let kw =
      Keyword.decorate
        (Token.pp ~inside:pincl_loc ~before:incl INCLUDE)
        ~extension attrs
    in
  ...
  let rec group_by_desc acc = function
    | [] -> [ List.rev acc ]
    | i :: is ->
        if same_group i (List.hd acc) then group_by_desc (i :: acc) is
        else List.rev acc :: group_by_desc [ i ] is
...
and Signature : sig
  ...
  let ends_in_obj lst =
    ...
    | Some { psig_desc; _ } -> (
        ...
        | Psig_module _ | Psig_recmodule _ | Psig_modsubst _ | Psig_modtype _
        | Psig_modtypesubst _ | Psig_open _ | Psig_include _ | Psig_attribute _
        | Psig_extension _ | Psig_class _ | Psig_class_type _ ->
            false)
        ...
        (fun md ->
          ...
          let keyword =
            ...
            let decorate tok =
              ...
              Keyword.decorate
                (Token.pp ~inside:md.pmd_loc ~before:fake_name tok)
                ~extension attrs
            in
            if !i = 0 then
          ...
          incr i;
          let binding =
            Binding.Module.pp ~loc:md.pmd_loc
              (Module_declaration.pp_raw md)
              ~context:(Module_declaration.decide_context md.pmd_type)
              ~keyword
          in
          Attribute.prepend_text text binding)
        mds
  ...
  let pp_item
      ({ psig_desc; psig_ext_attributes = ext_attrs; psig_loc = loc } as _item)
      =
    match psig_desc with
...
and Value_description : sig
  ...
  let pp ~ext_attrs:(extension, attrs) vd =
      ...
      | p :: ps ->
          let prims =
            separate_map (break 1)
              ~f:(fun { loc; txt } -> Constant.pp_string_lit ~loc txt)
              p ps
          in
    ...
    let kw =
      Keyword.decorate
        (Token.pp ~inside:vd.pval_loc ~before:name tok)
        ~extension attrs
    in
...
and Type_extension : sig
  ...
  let constructors ~loc = function
    ...
    | c :: cs ->
        case_list ~loc
          (Constructor_decl.pp_extension c)
  ...
  let pp ~ext_attrs:(extension, attrs)
    ...
    let keyword =
      Keyword.decorate
        (Token.pp ~inside:ptyext_loc ~before:lhs TYPE)
        ~extension attrs
    in
  ...
  let ends_in_obj = function
    | { ptyext_attributes = []; ptyext_constructors; _ } -> (
        match list_last ptyext_constructors with
...
and Type_exception : sig
  ...
  let pp ~ext_attrs:(extension, attrs)
    ...
    let kw =
      Keyword.decorate
        (Token.pp ~inside:ptyexn_loc ~before:cstr EXCEPTION)
        ~extension attrs
    in
...
and Type_declaration : sig
  ...
  type keyword = With_type of document | Type of rec_flag | And
  ...
  let with_params ~decl_loc ?(always_enclosed = false) ?(enclosing = parens) lst
      name =
    match lst with
    | [] -> name
  ...
  let add_constraints decl = function
    ...
    | cstr :: cstrs ->
        ...
        prefix ~indent:2 ~spaces:1 decl (kw ^/^ hang 2 (break 0 ^^ cstrs))
  ...
  let pp ?override_name ?binder ~keyword
      {
        ...
        ptype_private;
        ptype_manifest;
        ptype_ext_attributes = extension, attrs;
        ptype_attributes;
        ptype_loc;
      } =
    ...
    let keyword =
      ...
      | With_type previous_tok ->
          let tok =
            Keyword.decorate
              (Token.pp ~after:previous_tok ~before:lhs TYPE)
              ~extension attrs
          in
      ...
      | Type rf -> (
          ...
          | Recursive -> tok
          | Nonrecursive ->
              group (tok ^/^ Token.pp ~after:tok ~before:lhs NONREC))
    in
    match rhs with
    | Some rhs ->
...
and Class_type : sig
  ...
  let pp_constr ~loc name args =
    ...
    | x :: xs ->
        ...
        in
        let before = if Core_type.starts_with_obj x then break 1 else empty in
        let args =
...
and Class_expr : sig
  ...
  and pp_fun ~loc ~attrs params ce =
    let params =
      separate_map (break 1) ~f:Fun_param.pp (List.hd params) (List.tl params)
    in
    let body = pp ce in
...
and Class_structure : sig
  ...
  let pp_field_kind ~loc ~ext_attrs kind name decorate_keyword = function
    ...
    | Cfk_concrete (override, params, cts, expr) ->
        pp_concrete ~loc ~ext_attrs kind name decorate_keyword override params
          cts expr
  ...
  let pp_field_desc ~loc ~ext_attrs = function
    | Pcf_inherit (override, ce, alias) ->
        pp_inherit ~loc ~ext_attrs override ce alias
  ...
  let pp_field { pcf_desc; pcf_loc; pcf_ext_attributes; pcf_attributes } =
    let doc =
      pp_field_desc ~loc:pcf_loc ~ext_attrs:pcf_ext_attributes pcf_desc
    in
  ...
  let pp ~(loc : Location.t) ?ext_attrs:(extension, attrs = (None, []))
    ...
    let obj_with_self =
      ...
      | Some pcstr_self ->
          ...
          let obj =
            Keyword.decorate
              (Token.pp ~inside:loc ~before:self OBJECT)
              ~extension attrs
          in
          group (obj ^/^ self)
    in
      ...
      List.concat_map
        (fun cd ->
          let {
            ...
            pci_type;
            pci_expr;
            pci_ext_attributes = extension, attrs;
            pci_loc;
            pci_attributes;
          } =
            cd
    ...
    let cds =
      ...
      List.concat_map
        (fun cd ->
          ...
          in
          let lhs =
            Type_declaration.with_params ~always_enclosed:true
              ~enclosing:brackets ~decl_loc:pci_loc pci_params (str pci_name)
          in
...
let interface sg =
  let doc =
    match sg with [] -> empty | si :: sg -> Signature.pp_nonempty si sg
  in
  Document.attach_surrounding_comments doc

let implementation str =
  let doc =
    match str with [] -> empty | si :: st -> Structure.pp_nonempty si st
  in
  Document.attach_surrounding_comments doc
