...
module Rhs = struct
  type t = Absent | Regular of document | Two_parts of document * document

  let of_opt f = function None -> Absent | Some x -> Regular (f x)
end
...
let pp ?(binder = Source_parsing.Parser.EQUAL) ?keyword
  ...
  let with_coercion =
    attach_annot pre with_constraint ~sep:COLONGREATER coerce
  in
...
let pp_simple ?binder ~keyword lhs rhs =
  pp ?binder ~keyword
    { lhs; params = []; constr = None; coerce = None; rhs = Regular rhs }

module Module = struct
  type constraint_ = None | Sig of document | Mty of document
  type body = Items of document | Generic of document
  ...
  let located_body = function Generic doc | Items doc -> doc
  ...
  let pp ~keyword ~loc ~context { name; params; constr; body; attributes } =
    ...
    let with_constraint, binder =
      ...
      | Sig sg, Struct ->
          ...
          in
          let doc = group (params ^^ nest 2 sep) ^^ nest 2 (hardline ^^ sg) in
    ...
    let binder, rhs =
      ...
      | Items items ->
          let doc =
            let end_ =
              Token.pp ~inside:loc ~after:items ~before:attributes END
            in
