...
type ws_kind = Space | Hardline
type doc = Empty | WS of PPrint.document | Located of PPrint.document loc

type t = { doc : doc; left : border_info; right : border_info }
and border_info = { nest : int; ws : ws_kind option }
...
let repeat n d =
  let rec aux n doc = if n = 1 then doc else PPrint.(doc ^^ aux (n - 1) doc) in
  match d.doc with
  | Empty -> d
  ...
  ( final_loc,
    ...
    | left_cmts, right_cmts -> (
        ...
        | Some Hardline, Some Hardline ->
            ...
            ^^ layout left.nest ~before:(-1) right_cmts
            ^^ d2
        | Some Hardline, Some _ | Some _, Some Hardline ->
            d1
            ^^ layout right.nest left_cmts
            ^^ layout left.nest ~before:(-1) right_cmts
            ^^ d2
...
let ( ++ ) doc t =
  match t.doc with
  | Empty | WS _ -> invalid_arg "Document.(++)"
  | Located d ->
...
let suffix ~after:t c =
  match t.doc with
  | Empty | WS _ -> invalid_arg "Document.suffix"
  | Located doc ->
...
let collate_toplevel_items lst =
  let rec insert_blanks = function
    ...
    | x1 :: x2 :: rest ->
        let requirement x =
          match x.doc with
          | WS doc | Located { txt = doc; _ } -> PPrint.requirement doc
          | Empty -> 0
        in
        `Doc x1
        :: (if
  ...
  let rec join = function
    ...
    | otherwise -> otherwise
  in
  match join (insert_blanks lst) with [ `Doc x ] -> x | _ -> assert false
...
let separate_map sep ~f doc docs = separate sep (f doc) (List.map f docs)
...
let prefix ~indent ~spaces x y = group (x ^^ nest indent (break spaces ^^ y))
...
let flow sep first rest = flow_map sep (fun x -> x) first rest
...
let fmt_comments before = function
  ...
  | c :: cs ->
      let bi = { nest = 0; ws = None } in
      let cmt x =
        { doc = Located (Comment.comment x); left = bi; right = bi }
      in
