...
let is_docstring s =
  ...
  | "*" ->
      ...
      false
  | _ -> String.get s 0 = '*' && String.length s >= 2 && String.get s 1 <> '*'
...
let set_next ~next = function Null -> () | Cell cell -> cell.next <- next
let set_prev ~prev = function Null -> () | Cell cell -> cell.prev <- prev
...
let rec insert_comment prev txt loc = function
  ...
  | Cell t as cell ->
      ...
      if start < 0 then add_before ~prev txt loc ~next:cell
      else
...
let populate rest =
  List.fold_left
    (fun lst (txt, loc) ->
      if keep txt then insert_comment Null txt loc lst else lst)
    Null rest

let comments = ref Null
let init l = comments := populate l
