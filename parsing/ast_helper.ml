...
module Ci = struct
  let mk ?(loc = !default_loc) ?(ext_attrs = (None, [])) ?(attrs = [])
      ?(docs = empty_docs) ?(text = []) ?(virt = Concrete) ?(params = []) name
      term_params typ expr =
    {
      pci_virt = virt;
      pci_params = params;
      pci_name = name;
...
module Type = struct
  let mk ?(loc = !default_loc) ?(ext_attrs = (None, [])) ?(attrs = [])
      ?(docs = empty_docs) ?(text = []) ?(params = []) ?(cstrs = [])
      ?(kind = Ptype_abstract) ?priv ?manifest name =
    {
      ptype_name = name;
      ptype_params = params;
      ptype_cstrs = cstrs;
