...
let foo =
  ...
  [
    field "id" ~doc:"Toy ID." ~args:[] ~typ:(non_null guid) ~resolve:(function
        | _ctx -> x.id);
    field "id" ~doc:"Toy ID." ~args:[] ~typppp ~resolve:(function _ctx -> x.id);
    field "id" ~doc:"Toy ID." ~args:[] ~typ:(non_null guid) ~resolve:(function
        | A -> x.id
        | B -> c);
    field "id" ~doc:"Toy ID." ~args:[] ~resolve:(function A -> x.id | B -> c);
    field
      "id"
      ~doc:"Toy ID."
      ~args:[]
...
let foo =
  ...
  [
    ...
    field "type" ~doc:"Toy type. Possible values are: car, animal, train."
      ~args:[] ~typ:(non_null toy_type_enum) ~resolve:(fun _ctx x -> x.toy_type);
